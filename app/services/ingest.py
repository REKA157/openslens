...
async def handle_waha_event(payload: dict[str, Any]) -> dict:
    ...
    # 5. Médias + analyse de contenu (photo→vision, PDF/Office→lecture,
    # vocal→transcription). Renvoie l'analyse pour fusion dans la classification.
    media_analysis = None
    if _has_media(data):
        media_row = _build_media_row(message_uuid, data)
        sb.table("whatsapp_media").insert(media_row).execute()
        try:
            media_analysis = await media_service.download_and_store(
    ...
    # 6. Classification IA — fusion du texte + du CONTENU de la pièce jointe.
    # Permet de comprendre les messages dont le sens dépend de la pièce jointe
    # (« on fait quoi ? » + photo d'incident, bon de pesée en PDF, vocal du
    # chauffeur…) et de classer aussi les pièces jointes sans texte.
    caption = (msg_row.get("raw_text") or "").strip()
    classification_input = classify_service.build_media_aware_input(caption, media_analysis)
    if classification_input:
        try:
            await classify_service.classify_message(
                message_uuid, classification_input, media_context=media_analysis
            )
        except Exception as exc:  # noqa: BLE001
