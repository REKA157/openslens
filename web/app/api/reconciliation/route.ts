/** Proxy /api/reconciliation → backend /api/reconciliation (bypass cert self-signed). */
...
export async function GET(req: Request) {
  ...
  for (const k of ["date", "period"]) {
    const v = incoming.searchParams.get(k);
    if (v) qs.set(k, v);
  }
  const targetUrl = `${BACKEND_URL}/api/reconciliation${qs.toString() ? `?${qs}` : ""}`;
  try {
